...
/**
 * create the board 3x3 of 3x3 boards
 * create the list of open numbers
 * create hide logic for completd number
 *
 * write the legal / illegal move logic
 *    across full row
 *    down full col
 *    in mini board
 * Add UI to highlight why a move isn't allowed
 *
 * write the logic for initializing board (has to be solvable)
 *  levels: flash, easy, medium, hard, extra hard
 *
 * mini board
 *    must have 1 of each number
 *    cannot have more than 1 instance of the number
 *
 *    top array
 *    mid array
 *    bottom array
 ...
 *
 * full board
 *    must have 1 instance of each number per row and column
 *    cannot have duplicate numbers in same row and col
 *
 *    top miniboard array
 *    mid miniboard array
 *    bottom miniboard array
 *
 *    validateRow(num, row)
 *      return [...row[0].top, ...row[1].top, ...row[2].top].filter((el) => el === num).length === 0;
 *
 *    validateCol(num, matrixIdx, colIdx)
 *      return [...top[matrixIdx].map, ...mid[matrixIdx].top, ...bottom[matrixIdx].top].filter((el) => el === num).length === 0;
 *
 * [
 *  [[], [[], [[],
 *   [],  [],  [],
